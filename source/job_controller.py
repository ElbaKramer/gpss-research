...
def evaluate_models(models, X, y, verbose=True, iters=300, local_computation=False, zip_files=False, max_jobs=500, random_seed=0, subset=False, subset_size=250, full_iters=0, bundle_size=1, relational=False):
    ...
    for (i, model) in enumerate(models):
        parameters = {'datafile': data_file.split('/')[-1],
                      ...
                      'mean_syntax': model.mean.get_gpml_expression(dimensions=XX.shape[1] if relational else X.shape[1]),
                      'mean_params': '[ %s ]' % ' '.join(str(p) for p in model.mean.param_vector),
                      'kernel_syntax': model.kernel.get_gpml_expression(dimensions=XX.shape[1] if relational else X.shape[1]),
                      'kernel_params': '[ %s ]' % ' '.join(str(p) for p in model.kernel.param_vector),
                      'lik_syntax': model.likelihood.get_gpml_expression(dimensions=XX.shape[1] if relational else X.shape[1]),
